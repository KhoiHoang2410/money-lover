...
enum CalendarMath {
    ...
    /// (they enrich history but never the computed totals). Foreign-currency entries are excluded
    ...
    static func dailyNet(transactions: [Transaction], year: Int, month: Int, calendar: Calendar = .current) -> [Int: Money] {
        var totals: [Int: Int] = [:]
        for transaction in transactions where transaction.affectsBalance && transaction.amount.currency == .vnd {
